...
public class GUI {
	...
	public void openSignInForm(JFrame frame, Login_Account user) {
		...
		//f.setBackground(Color.gray);
                f.setBackground(new Color(176, 241, 240));
		
		// main screen picture
		JLabel imgLabel = new JLabel(new ImageIcon( System.getProperty("user.dir") + "\\src\\def_pkg\\logo1.jpg"));
		//imgLabel.setBounds(20,24,450, 400);
                imgLabel.setBounds(20,24,450, 400);
		f.add(imgLabel);
		
		//---------------------------------Creating Labels------------------------------------------		
		//JLabel l_hSignIn = new JLabel("Amar Bank");
                JLabel l_hSignIn = new JLabel(new ImageIcon( System.getProperty("user.dir") + "\\src\\def_pkg\\AMB1.png"));
		//l_hSignIn.setFont( l_hSignIn.getFont().deriveFont(30f) );
		l_hSignIn.setBounds(500,50,372, 67);
               // l_hSignIn.setForeground(Color.BLACK);
		f.add(l_hSignIn);
		...
		btn_SignIn.addActionListener((ActionEvent e) -> {
                    ...
                    Login_Account user1 = db.signIn( tf_UserName.getText(), tf_Password.getText() );
                    if (user1.getLoginId().compareTo("") == 0) {
                        JOptionPane.showMessageDialog(f, "Wrong username or password");
                    } else {
                        System.out.println("Login type=" + user1.getType());
                        if (user1.getType().compareTo("Client") == 0) {
                            Client client = db.getClient(user1.getLoginId());
                            Bank_Account account = db.getAccount(user1.getLoginId());
                            switch (Integer.valueOf(account.getStatus())) {
                                case 0:
                        ...
                        } else if (user1.getType().compareTo("Manager") == 0) {
                            Manager manager = new Manager(user1.getName());
                            frame.remove(f);
                            frame.repaint();
                            frame.validate();
                            openManagerMenu(frame, manager);
                        } else if (user1.getType().compareTo("Accountant") == 0) {
                            Accountant accountant = new Accountant(user1.getName());
                            frame.remove(f);
                            frame.repaint();
                            frame.validate();
	...
	void open_Signup_form_1(JFrame frame, Login_Account user)
	{
		...
		JLabel l_CNIC = new JLabel("Enter the NID:");
		...
		btn_Verify_Account.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
					...
					{
						JOptionPane.showMessageDialog(f, "No account found with this NID");
						// **************************************
						System.out.println("The NID did not match with any client");
						frame.remove(f);
						frame.repaint();
						frame.validate();
						openSignInForm(frame, user);
...
/*
		...
		f.add(btnCCWithdrawal);
                btnCCWithdrawal.setEnabled(false);
...
*/
		...
		btnTransferMoney.setBounds(300,180,200, 40);
	...
	void open_cardless_page_1(JFrame frame, Client client, Bank_Account account)
	{
		...
		JLabel l_temp_pin = new JLabel("Enter the 4-digit  pin:");
		...
		f.add(btn_create_cardless_entry);
                btn_create_cardless_entry.setEnabled(false);
	...
	void openTransferMoneyForm(JFrame frame, Client client, Bank_Account account) {
		...
		btn_Transfer.addActionListener((ActionEvent e) -> {
		...
		btn_sign_out.addActionListener((ActionEvent e) -> {
                    frame.remove(f);
                    frame.repaint();
                    frame.validate();
                    Login_Account user = new Login_Account();
	...
	void eStatement2(JFrame frame, Client client, Bank_Account account) {
	    ...
	    // Day, Month and Year for From Date
 	    ...
 	    f.add(toDaySpinner); 
 	    SpinnerModel toMonthValues =  new SpinnerNumberModel(06, 1, 12, 1);  
 	    JSpinner toMonthSpinner = new JSpinner( toMonthValues);   
 	    toMonthSpinner.setBounds(500,130,40,25);    
 	    f.add(toMonthSpinner); 
 	    SpinnerModel toYearValues =  new SpinnerNumberModel(2023, 2000, 2030, 1);  
		...
		btnEStatement.addActionListener((ActionEvent e) -> {
	...
	void openManagerMenu(JFrame frame, Manager manager) {
		...
		// function to be executed  Button is clicked
		...
		btn_create.addActionListener((ActionEvent e) -> {
                    ...
                    switch (res) {
                        case 1:
                            ...
                            break;
                        case 2:
                            JOptionPane.showMessageDialog(f, "Another account with this NID number exists");
                            break;
                        default:
	...
	void open_manage_card_page(JFrame frame, Manager manager)
	{
		...
		//		Account Opening Date
		JLabel l_cnic = new JLabel("Enter NID of account holder");
	...
	void open_block_unblock_account_page(JFrame frame, Manager manager)
	{
		...
		JLabel l_cnic = new JLabel("Enter NID:");
		...
		btnSearch.addActionListener( new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				// Both client and his account was found
					...
					lCCNIC.setText("NID:  "+client.getCNIC() );
	...
	void searchForm(JFrame frame, Accountant accountant, int case_) {
		...
		// CNIC Number Label
		JLabel lCnic = new JLabel("Enter NID number:");
	...
	void depositCash(JFrame frame, Accountant accountant, Client client, Bank_Account account) {
		...
		// CNIC Label
		JLabel lCnic = new JLabel( "NID: " + client.getCNIC() );
	...
	void eStatement(JFrame frame, Accountant accountant, Client client, Bank_Account account) {
	    ...
	    f.add(fromMonthSpinner); 
	    SpinnerModel fromYearValues =  new SpinnerNumberModel(2021, 2000, 2030, 1);  
	    JSpinner fromYearSpinner = new JSpinner(fromYearValues);   
		...
		// function to be executed when Main Menu Button is clicked
		btn_mm.addActionListener((ActionEvent e) -> {
                    frame.remove(f);
                    frame.repaint();
                    frame.validate();
                    openAccountantMenu(frame, accountant);
		...
		btnDeposit.addActionListener(new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        if( Integer.parseInt( tfAmount.getText() ) > 0 ) {
                            int z = accountant.chequeDeposit(tfAccNum.getText(), tfChequeNum.getText(), Integer.parseInt( tfAmount.getText() ) );
                            switch (z) {
                                case 2 -> JOptionPane.showMessageDialog(f, "Account not found");
                                case 3 -> JOptionPane.showMessageDialog(f, "Account is blocked");
                                case 4 -> JOptionPane.showMessageDialog(f, "Account is closed");
                                default -> {
                                    if( z == 1 )
